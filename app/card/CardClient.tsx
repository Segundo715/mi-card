...
interface CardClientProps {
  initialCfg: CafeConfig
  initialBgColor: string
  initialBtnColor: string
}

// La identidad de marca (cfg/bgColor/btnColor) llega ya resuelta desde el Server Component
// (page.tsx) — antes se pedía con un fetch client-side a /api/settings DESPUÉS del primer
// paint, así que el logo del hero no empezaba a descargarse hasta que esa respuesta llegaba.
// Ahora el <img> del logo va en el HTML inicial y el navegador lo pide en paralelo con el
// resto de la página, no después. Lo único que sigue siendo client-side es lo que depende de
// localStorage (si ya hay una tarjeta registrada en este dispositivo).
export function CardClient({ initialCfg, initialBgColor, initialBtnColor }: CardClientProps) {
  const [step, setStep] = useState<Step>('loading')
  const [customer, setCustomer] = useState<Customer | null>(null)
  const [cfg] = useState<CafeConfig>(initialCfg)
  ...
  const [btnColor] = useState(initialBtnColor)
