...
// Las categorías (con marca ya resuelta) llegan del Server Component (page.tsx) — antes se
// pedían con un fetch client-side DESPUÉS del primer paint, así que los logos no empezaban a
// descargarse hasta que esa respuesta volvía. Lo único que sigue siendo client-side es la sesión
// ya iniciada (localStorage, no accesible desde el servidor).
export function WalletClient({ initialCategories }: { initialCategories: RewardCategory[] }) {
  const [customer, setCustomer] = useState<Customer | null>(null)
  const [categories] = useState<RewardCategory[]>(initialCategories)
