...
export interface Brand {
  color: string; logo: string; logoColor: string; brandText: string; brandLogo: string
}
...
// La marca (brand) llega ya resuelta desde el Server Component (page.tsx) — antes se pedía con un
// fetch client-side DESPUÉS del primer paint, así que el logo no empezaba a descargarse hasta que
// esa respuesta volvía. Ahora el <img> va en el HTML inicial. Lo único que sigue siendo
// client-side es la sesión ya iniciada (localStorage, no accesible desde el servidor).
export function UsuarioClient({ initialBrand }: { initialBrand: Brand }) {
